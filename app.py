    ...
    if time_df.empty:
        ...
        def create_combined_chart(data, group_col_name=None):
            ...
            if group_col_name:
                    ...
                    for group_val in sorted_groups:
                        ...
                        if not group_row.empty:
                            ...
                            cumulative_data.append({
                                ...
                                'percentage': percentage,  # 한글 대신 영문 컬럼명 사용
                                'mid_y': mid_y  # 중점 위치
                            })
                            cumsum = end_y
                ...
                # 표시 모드에 따른 세그먼트 텍스트 차트 생성
                if label_display_mode == "비중(%)":
                    # 비중 표시: "12.3%" 형태로 표시
                    mid_point_df['display_text'] = mid_point_df['percentage'].apply(lambda x: f"{x:.1f}%" if x > 0 else "")
                    ...
                    segment_text = alt.Chart(mid_point_df).mark_text(
                        ...
                        y=alt.Y('mid_y:Q', 
                               ...
                               scale=alt.Scale(domain=[0, expanded_max_amount])),
                        text=alt.condition(
                            alt.datum.percentage >= 5,  # 5% 이상만 표시
                    ...
                    segment_text = alt.Chart(mid_point_df).mark_text(
                        ...
                        y=alt.Y('mid_y:Q', 
                               ...
                               scale=alt.Scale(domain=[0, expanded_max_amount])),
                        text=alt.condition(
                            alt.datum.송장금액_백만원 >= 20,  # 20 백만원 이상만 표시
