...
def _standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 동일한 헤더가 두 번 이상 존재할 경우 첫 번째만 남김
...
def load_csv(upload: BytesIO) -> pd.DataFrame:
    ...
    # 숫자형 컬럼 정리
    numeric_cols = [
        col for col in ["송장수량", "송장금액", "단가", "플랜트", "구매그룹"] if col in df.columns
    ]
    df[numeric_cols] = (
        df[numeric_cols]
...
def str_list_to_sql(values: list[str]) -> str:
    """문자열 리스트 ➜ SQL IN 절 문자열 (SQL escape 포함)"""
...
if df is not None and not df.empty:
    ...
    with st.sidebar:
        ...
        plants_all = (
            df["플랜트"].dropna().astype(int).sort_values().unique().tolist()
            if "플랜트" in df.columns else []
        )
        groups_all = (
            df["구매그룹"].dropna().astype(int).sort_values().unique().tolist()
            if "구매그룹" in df.columns else []
        )
        suppliers_all = (
            df["공급업체명"].dropna().sort_values().unique().tolist()
            if "공급업체명" in df.columns else []
        )

        selected_years = st.multiselect("연도", years_all, default=years_all)
        selected_plants = (
            st.multiselect("플랜트", plants_all, default=plants_all) if plants_all else []
        )
        selected_groups = (
            st.multiselect("구매그룹", groups_all, default=groups_all) if groups_all else []
        )
        selected_suppliers = (
            st.multiselect("공급업체", suppliers_all, default=suppliers_all) if suppliers_all else []
        )
    ...
    if suppliers_all:
        ...
        if not sup_df.empty:
            ...
            st.download_button(
                "업체별 CSV 다운로드", sup_csv, file_name="supplier_summary.csv", mime="text/csv"
            )
